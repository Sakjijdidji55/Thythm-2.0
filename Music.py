...
class MUSICPAUSER:
    ...
    def draw(self, window):
        ...
        window.blit(big_return_img, (WIDTH // 3 + WIDTH // 12 - 36, HEIGHT // 3 + HEIGHT // 6 - 36))
    ...
    def check(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if event.button == 1:
                ...
                if WIDTH // 3 + WIDTH // 12 - 36 <= pos[0] and pos[
                    0] <= WIDTH // 3 + WIDTH // 12 + 36 and HEIGHT // 3 + HEIGHT // 6 - 36 <= pos[1] and pos[
                    1] <= HEIGHT // 3 + HEIGHT // 6 + 36:
                    # 设置选项为已选择
                    self.choice = True
                    return 'return'
                # 检查鼠标点击的位置是否在第二个选项的范围内
                elif WIDTH // 2 + WIDTH // 12 - 36 <= pos[0] and pos[
                    0] <= WIDTH // 2 + WIDTH // 12 + 36 and HEIGHT // 3 + HEIGHT // 6 - 36 <= pos[1] and pos[
                    1] <= HEIGHT // 3 + HEIGHT // 6 + 36:
                    # 设置选项为已选择
                    self.choice = True
                    # 返回'continue'
...
class music:
    ...
    def __init__(self, music_name: str, music_path: str, music_volume: float, music_writer: str, rank: str):
...
class MUSIC(music):
    ...
    def load(self):
        ...
        self.cover = pygame.transform.scale(pygame.image.load(musics[deal_name(self.music_name)]["cover"]), (WIDTH//2, HEIGHT//2))
            ...
            for ball in self.longBalls:
                if ball.ischeck:
                    if ball.ischeck % 10 == 0:
                        self.continue_bit += 1
                        self.particles.extend(make_particles(ball.x + 25 / 1536 * WIDTH, TEST_Y + 25*WIDTH//1536, green if ball.score == 2 else blue))
                    ball.ischeck += 1
        ...
        if event.type == pygame.KEYDOWN: # 检测是否有键被按下
            ...
            for ball in self.shortBalls:
                ...
                if check and cnt:
                    ...
                    if check == 1:
                        self.good += 1
                        self.particles.extend(make_particles(ball.x+25/1536*WIDTH, TEST_Y + 25*WIDTH//1536, blue))
                        self.shortBalls.remove(ball)
                        del ball
                    elif check == 2:
                        self.prefect += 1
        ...
        while time.time() - start_time < 3:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    sys.exit()
            obj.draw(window)
            text = f.render(str(int(4 - time.time() + start_time)), True, (255, 255, 255))
    ...
    def is_check_setting(self, event, window):
        if event.type == pygame.MOUSEBUTTONDOWN:
            pos = pygame.mouse.get_pos()
            if 25 / 1536 * WIDTH <= pos[0] <= 97 / 1536 * WIDTH and pos[1] >= 40 / 1536 * WIDTH and pos[
                1] <= 112 / 1536 * WIDTH and event.button == 1: # 检测是否点击了暂停按钮
                self.pause() # 暂停
