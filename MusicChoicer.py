...
class MusicChoicer:
    def __init__(self,songs_list,theme,father):
        ...
        self.bk_pos = (700/1536*WIDTH,216/1536*WIDTH) # 通过计算得到的背景图片位置
    ...
    def draw(self, window):
        window.blit(self.songs_list[self.index[1]].image, (0, 0)) # 添加背景图片，为当前音乐的背景
        ...
        pygame.draw.line(window, self.line_color, self.line_pos[0], self.line_pos[1], 4) # 添加分割线
        pygame.draw.line(window, self.line_color, self.line_pos[1], self.line_pos[2], 4) # 添加分割线
        ...
        for i in range(4):
            ...
            writer_surface = self.text_size.render(writer, True, self.text_color) # 渲染文字
            window.blit(index_surface, self.text_positions[i][0])
            window.blit(name_surface, self.text_positions[i][1])
            window.blit(writer_surface, self.text_positions[i][2]) # 一个界面中显示4首歌曲，每首歌曲显示序号、歌名、作者，按照事先算的坐标
    ...
    def scroll(self,event):
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 5:
                ...
                if (WIDTH // 2 - 200 / 1536 * WIDTH < pos[0] < WIDTH // 2 + 200 / 1536 * WIDTH and HEIGHT // 8 - 65 / 1536 * WIDTH < pos[1] < HEIGHT // 8 + 35 / 1536 * WIDTH) or (
                        25 / 1536 * WIDTH < pos[0] < 97 / 1536 * WIDTH and 40 / 1536 * WIDTH < pos[1] < 112 / 1536 * WIDTH) :
                    self.songs_list[self.index[1]].stop_sound()
