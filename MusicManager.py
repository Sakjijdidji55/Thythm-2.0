...
class theme:
    def __init__(self,name: str,cover: str,image: str,music_list: list,father: object):
        '''
        name: 主题名称
        cover: 主题图片
        image: 主题背景图片
        ...
        '''
        self.name = name
        self.cover = pygame.transform.scale(pygame.image.load(cover), (WIDTH//2,HEIGHT//2))
...
class ThemeManager:
    ...
    def init_themes(self):
        ...
        for source in self.theme_sources:
            self.theme_list.append(theme(source[0],source[1],source[2],source[3],self)) # 创建主题对象
    ...
    def draw(self,window: pygame.Surface):
        ...
        window.blit(right,(WIDTH-200/1536*WIDTH,HEIGHT//2+HEIGHT//8 - 50/1536*WIDTH)) # 左右按钮
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.button == 1:
                if pos[0] > 100/1536*WIDTH and pos[0] < 200/1536*WIDTH and pos[1] > HEIGHT//2+HEIGHT//8 - 50/1536*WIDTH and pos[1] < HEIGHT//2+HEIGHT//8 + 50/1536*WIDTH:
                    self.switch(1,window)
                    self.current_theme_index = (self.current_theme_index - 1) % len(self.theme_list) # 点到了左边,向左切换
                    self.switch(2,window)
                    ...
                    self.switch(1,window)
                    self.current_theme_index = (self.current_theme_index + 1) % len(self.theme_list) # 点到了右边，向右切换
                    self.switch(2,window)
                if pos[0] > WIDTH // 4 and pos[0] < WIDTH // 4 +  WIDTH // 2 and pos[1] > HEIGHT // 4 + HEIGHT // 8 and pos[1] < HEIGHT // 4 + HEIGHT // 8 + HEIGHT // 2: # 点击了主题图片或者按下回车键
                    threading.Thread(target=lambda : enter_music_effect.play()).start()
                    self.sound.stop()
                    return self.get_current_theme_MusicChoicer()
