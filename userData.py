...
def get_info(username_entry, description_text, file_path_label):
    ...
    description = description_text.get("1.0", tk.END).strip() # 获取描述,strip()函数用于去除字符串两端的空白字符，get("1.0", tk.END) 用于获取文本框中的所有文本
    user_inform["description"] = description # 赋值描述
    file_path = file_path_label.cget("text").replace("选择的头像文件路径: ", "") # 获取文件路径
    if file_path:
...
def set_user_inform(user_inform_path):
    ...
    save_user_inform(user_inform_path,user_inform) # 保存用户信息，将用户的头像路径放入自己数据库
